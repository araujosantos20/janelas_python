...
class AtualizarClientes(QWidget):
    ...
    def upCli(self):
        if(self.editId.text()==""):
            ...
            cursor.execute("update clientes set nome_cliente=%s where cliente_id=%s",
                           (self.editNome.text(),self.editId.text()))
            

        elif(self.editNome.text()=="" and self.editEmail.text()!="" and self.editTelefone.text()==""):
            cursor.execute("update clientes set email=%s where cliente_id=%s",
                           (self.editEmail.text(),self.editId.text()))
            
        elif(self.editNome.text()=="" and self.editEmail.text()=="" and self.editTelefone.text()!=""):
            cursor.execute("update clientes set telefone=%s where cliente_id=%s",
                           (self.editTelefone.text(),self.editId.text()))
            
        else:
            cursor.execute("update clientes set nome_cliente=%s, email=%s, telefone=%s where cliente_id=%s",
